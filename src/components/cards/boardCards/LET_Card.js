    ...
    render() {
        ...
        return (
            <div style={cardStyle} onClick={log} className='Card'>
                ...
                <div>
                    <ul style={{fontSize:'7px', textAlign:'left', fontSize: '11px', listStylePosition: 'inside'}}>
                        {enhancements.map((item, index) => (<li key={index}>{item}</li>))}
                    </ul>
                </div>

                <div style={contentStyle} name='content'>
                    <div style={{fontSize:'12px'}}>Learning Analytics</div>
                ...
                <div name='Description'>
                    <div style={{ fontSize: '13px',  margin: '0px', textAlign: 'center' }}>Learning Enhancing Technology</div><div style={{fontSize:'7px', margin: '0px'}}>(code:{code})</div>
